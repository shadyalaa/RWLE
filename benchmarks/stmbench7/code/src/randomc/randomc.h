...
*
* void RandomInit(uint32_rnd seed);
* Re-initializes the random number generator with a new seed.
*
* void RandomInitByArray(uint32_rnd seeds[], int length);
...
typedef   signed long int32_rnd;     
typedef unsigned long uint32_rnd;     
...
class TRandomMersenne {                // encapsulate random number generator
  ...
  #endif
  public:
  TRandomMersenne(uint32_rnd seed) {       // constructor
    RandomInit(seed);}
  void RandomInit(uint32_rnd seed);        // re-seed
  void RandomInitByArray(uint32_rnd seeds[], int length); // seed by more than 32 bits
  int IRandom(int min, int max);       // output random integer
  double Random();                     // output random float
  uint32_rnd BRandom();                    // output random bits
  private:
  uint32_rnd mt[MERS_N];                   // state vector
...
class TRanrotBGenerator {              // encapsulate random number generator
  ...
  public:
  void RandomInit(uint32_rnd seed);        // initialization
  int IRandom(int min, int max);       // get integer random number in desired interval
  double Random();                     // get floating point random number
  TRanrotBGenerator(uint32_rnd seed);      // constructor
  protected:
  int p1, p2;                          // indexes into buffer
  uint32_rnd randbuffer[KK];               // history buffer
  uint32_rnd randbufcopy[KK*2];            // used for self-test
  enum TArch {LITTLE_ENDIAN1, BIG_ENDIAN1, NONIEEE};
  TArch Architecture;                  // conversion to float depends on computer architecture
};
...
class TRanrotWGenerator {              // encapsulate random number generator
  ...
  uint32_rnd BRandom();                    // output random bits  
  TRanrotWGenerator(uint32_rnd seed);      // constructor
  protected:
  int p1, p2;                          // indexes into buffer
  union {                              // used for conversion to float
    long double randp1;
    uint32_rnd randbits[3];};  
  uint32_rnd randbuffer[KK][2];            // history buffer
  uint32_rnd randbufcopy[KK*2][2];         // used for self-test
...
class TRandomMotherOfAll {             // encapsulate random number generator
  ...
  int IRandom(int min, int max);       // get integer random number in desired interval
  double Random();                     // get floating point random number
  TRandomMotherOfAll(uint32_rnd seed);     // constructor
  protected:
