...
#else
  _assert_(true);
  ::pthread_spinlock_t* mutex = new ::pthread_spinlock_t;
  if (::pthread_spin_init(mutex, NULL) != 0) throw std::runtime_error("pthread_mutex_init");
  opq_ = (void*)mutex;
#endif
}
...
#else
  _assert_(true);
  ::pthread_spinlock_t* mutex = (::pthread_spinlock_t*)opq_;
...
void Mutex::lock() {
  ...
  _assert_(true);
  ::pthread_spinlock_t* mutex = (::pthread_spinlock_t*)opq_;
  //if (::pthread_spin_lock(mutex) != 0) throw std::runtime_error("pthread_mutex_lock");
...
bool Mutex::lock_try() {
  ...
  _assert_(true);
  ::pthread_spinlock_t* mutex = (::pthread_spinlock_t*)opq_;
  //int32_t ecode = ::pthread_spin_trylock(mutex);
...
void Mutex::unlock() {
  ...
  _assert_(true);
  ::pthread_spinlock_t* mutex = (::pthread_spinlock_t*)opq_;
  //if (::pthread_spin_unlock(mutex) != 0) throw std::runtime_error("pthread_mutex_unlock");
  TM_END(mutex);
#endif
}
...
#else
  _assert_(true);
  //[000] switching spinlock to mutexes
  ::pthread_spinlock_t* mutex =  new ::pthread_spinlock_t;
  if (::pthread_spin_init(mutex, NULL) != 0) throw std::runtime_error("pthread_spin_init");
    opq_ = (void*)mutex;
#endif
}
...
RWLock::RWLock() : opq_(NULL) {
  ...
  opq_ = (void*)rwlock;*/
  //long counters[80];
  //long readers_counters [80];
  //readers_counters = counters;
  reader_locks = (volatile int*)calloc( 80, sizeof(volatile int) );
  //readers_counters = new long[80];
  //int x; for(x=0;x<80;x++) readers_counters[x]=0;
...
#else
  _assert_(true);
  //[000] switching spinrwlock to mutexes
  /*SpinRWLockCore* core = new SpinRWLockCore;
...
bool SpinRWLock::promote() {
  _assert_(true);
  //[000] switching spinrwlock to mutexes, no need for promote
  /*SpinRWLockCore* core = (SpinRWLockCore*)opq_;
  spinrwlocklock(core);
  if (core->rc > 1) {
    spinrwlockunlock(core);
...
void SpinRWLock::demote() {
  _assert_(true);
  //[000] switching spinrwlock to mutexes, no need for demote
  /*SpinRWLockCore* core = (SpinRWLockCore*)opq_;
  spinrwlocklock(core);
  core->wc--;
  core->rc++;
